...
async function migrateLegacyOfflineQueue() {
  ...
  try {
    ...
    if (queue && queue.length > 0) {
      console.log(
        `Migrating ${queue.length} legacy offline queue items to Firebase...`,
      );
      for (const item of queue) {
        const itemId = String(
          item.id || item.payload?.id || item.payload?.key || "",
        );
        if (!itemId) continue;
        ...
        try {
          if (
            ...
            await setDoc(doc(db, item.table, itemId), cleanPayload, {
              merge: true,
            });
...
export async function authenticateUserWithPIN(
  ...
  try {
    ...
    const user = data.users.find(
      (u) => String(u.id) === String(userId) && String(u.pin) === String(pin),
    );
...
export async function migrateLegacyCacheToFirebase() {
  ...
  try {
    ...
    console.log(
      "Migrating full legacy cache (APK offline data) to Firebase...",
    );
...
export async function loadAll(): Promise<NFCData> {
  ...
  try {
    ...
    const timeout = new Promise((_, reject) =>
      setTimeout(() => reject(new Error("Cloud fetch timeout")), 15000),
    );
    ...
    console.warn(
      "Firebase fetch timeout or network error... utilizing local unified cache.",
      e,
    );
...
export async function executeWrite<K extends keyof NFCData>(
  ...
  try {
    if (action === "insert" || action === "upsert" || action === "update") {
      ...
      setDoc(doc(db, table, itemId), cleanPayload, { merge: true }).catch((e) =>
        console.warn(`Silent online sync catch:`, e),
      );
    } else if (action === "delete") {
      deleteDoc(doc(db, table, itemId)).catch((e) =>
        console.warn(`Silent online sync delete catch:`, e),
      );
    }
  } catch (e) {
...
async function syncLocalCacheItem<K extends keyof NFCData>(
  ...
  switch (action) {
    case "insert":
      if (
        !tableList.some(
          (x) => String(x.id || (x as any).key) === String(itemId),
        )
      )
...
export function setupRealtimeSubscriptions(onUpdate?: () => void): () => void {
  ...
  tables.forEach((table) => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot: any) => {
        if (snapshot.docChanges().length > 0) {
          ...
          snapshot.docChanges().forEach((change: any) => {
            ...
            const tableList = (cache[table as keyof NFCData] || []) as any[];
            const itemId = String(record.id || record.key || "");

            switch (action as string) {
              case "insert":
                if (
                  !tableList.some(
                    (x: any) => String(x.id || x.key) === String(itemId),
                  )
                )
              ...
              case "upsert": {
                ...
                if (idx !== -1)
                  tableList[idx] = { ...tableList[idx], ...record };
                else tableList.push(record);
                break;
              }
              case "delete":
                cache[table as keyof NFCData] = tableList.filter(
                  (x: any) => String(x.id || x.key) !== String(itemId),
                ) as any;
                break;
            }
      ...
      (error: any) => {
        console.warn(
          `Firebase realtime subscription error on ${table}:`,
          error,
        );
      },
    );
