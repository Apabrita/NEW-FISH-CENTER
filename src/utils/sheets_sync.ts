...
export async function getSheetValues(
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `Failed to read from Google Sheet: ${errorDetails.error?.message || res.statusText}`,
    );
  }
 ...
 */
export async function importBuyersFromSheet(
  spreadsheetId: string,
): Promise<number> {
 ...
 */
export async function importSourcesFromSheet(
  spreadsheetId: string,
): Promise<number> {
...
export async function updateSheetRange(
  ...
  if (!res.ok) {
    const errorDetails = await res.json().catch(() => ({}));
    throw new Error(
      `Failed updating data: ${errorDetails.error?.message || res.statusText}`,
    );
  }
}
 ...
 */
export async function exportDatabaseToSheet(
  spreadsheetId: string,
  data: NFCData,
): Promise<number> {
  let rowsExported = 0;
  ...
  const buyersRows = data.buyers.map((b) => [
    b.id,
    b.nickname,
    b.lifetime_debt,
    b.credit_limit,
  ]);
  await updateSheetRange(spreadsheetId, "Buyers!A1", [
    buyersHeaders,
    ...buyersRows,
  ]);
  ...
  const sourcesRows = data.sources.map((s) => [
    s.id,
    s.name,
    s.rate_per_kg,
    s.date,
  ...
  await updateSheetRange(spreadsheetId, "Sources!A1", [
    sourcesHeaders,
    ...sourcesRows,
  ]);
  ...
  await updateSheetRange(spreadsheetId, "Transactions!A1", [
    txHeaders,
    ...txRows,
  ]);
...
export async function bidirectionalSync(
  ...
  try {
    ...
    console.warn(
      "Could not import Buyers (maybe sheet doesn't exist yet):",
      err,
    );
  }
