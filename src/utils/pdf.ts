...
export const shareAsPDF = async (
  ...
  const hiddenElements: HTMLElement[] = Array.from(
    clone.querySelectorAll(".print\\:hidden, button"),
  );
  ...
  try {
    ...
    const pageNodes = Array.from(
      clone.querySelectorAll(".print-page-wrapper"),
    ) as HTMLElement[];
    ...
    if (pageNodes.length > 0) {
      const masterHeaders = Array.from(
        clone.querySelectorAll(".pdf-master-header"),
      );
    ...
    alert(
      "Failed to generate PDF. Falling back to simple print... Error: " +
